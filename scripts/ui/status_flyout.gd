...
func _ready() -> void:
	...
	panel_container.size = Vector2(300, 250)
	panel_container.position = Vector2(-320, 10)  # Start off-screen to the left
	...
	label.custom_minimum_size = Vector2(280, 200)
...
Facing: %s
State: %s
""" % [hunger_text, worm_player.dirt_eaten_count, tile_type_text, facing_text, status_text]
	...
	if is_open:
		...
		panel_container.position = Vector2(-320, 10)
